...
For single pattern GAGGAGGCGG 
Length of each seq= 100
Number of seq=1000
@author: akriti
"""
import numpy as np
...
from utility import extract_motif_cls,get_convolved_op,get_decodedsig
...
def getCNNParams(): 
    fpath='Data/DNA_seq.fa'
    bpath='Data/DNA_backseq.fa'
                ...
                argument_1.update(row)
                 ...
                 if len(argument_2)<1:
                     ...
                     for x in temp:
                         arg_vals.add(x)
                         count+=1
        #print("count ",count," Filter num ",z)
        #print("Values of arguments ",arg_vals)
        if len(value)<1:
            value=arg_vals
            filter_num=z
        elif max(value)<max(arg_vals):
            value=arg_vals
            filter_num=z
    return filter_num,max(value)           
               
def printing_motif(filter_op,actual_X,arg_m):
...
def printing_motif_argm(filter_op,actual_X):
    pos=np.argmax(filter_op,axis=1)
    i=0
    for row in actual_X:
        val=int(pos[i])
        print(val)
...
conv_out=get_convolved_op(Xarr,kernel_full[-1][0],bias_full[-1][0])
filter_number,arg_m=finding_motif_pos(conv_out)
filter_op=conv_out[:,:,filter_number]
#printing_motif(filter_op,actual_X,arg_m)
printing_motif_argm(filter_op,actual_X)
